    ...
    <div className={`bg-white rounded-[20px] shadow-sm flex flex-col relative overflow-hidden transition-all border border-gray-100 hover:shadow-md`}>
        ...
        <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-start bg-gray-50/40">
           ...
           <div className="text-right">
               <span className="text-[11px] font-bold text-gray-600 bg-white border border-gray-100 shadow-sm px-2.5 py-1 rounded-md flex items-center gap-1.5">
        ...
        <div className="flex-1 p-5 space-y-4 bg-white">
            ...
            {order.items?.map((item, idx) => {
                // 🟢 ดึงชื่อหมวดหมู่มาใช้งาน (รองรับกรณีข้อมูลผูกผ่าน Backend Prisma)
                const categoryName = item.product?.category?.categoryName || item.categoryName || 'ทั่วไป';
                    ...
                    <div key={idx} className="flex gap-3 items-start border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                        ...
                        <div className="flex-1 pt-0.5">
                            <div className="flex justify-between items-start gap-2">
                                ...
                                {/* 🟢 ป้ายแสดงหมวดหมู่เมนู */}
                                <span className="text-[9px] font-black px-2 py-0.5 bg-gray-100 text-gray-600 rounded border border-gray-200 uppercase tracking-wider shrink-0">
                                    {categoryName}
                                </span>
                            </div>
        ...
        {/* ส่วนท้ายของการ์ด */}
        <div className="p-4 bg-gray-50/60 border-t border-gray-100 flex flex-col gap-3 mt-auto">
            <div className="flex justify-between items-center px-1">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">ช่องทางชำระเงิน</span>
                  {order.paymentMethod === 'transfer' ? (
                    <span className="text-[13px] font-bold text-blue-600 flex items-center gap-1.5"><QrCode size={16}/> โอนเงิน</span>
                  ) : (
                ...
                <button 
                  onClick={() => {
                      ...
                      if (slipPath) {
                          ...
                          Swal.fire({ title: 'ไม่พบสลิป', text: 'ลูกค้าไม่ได้แนบสลิปโอนเงินมาครับ', icon: 'info' });
                      }
                  }} 
                  className="w-full flex items-center justify-center gap-2 text-[13px] bg-white border border-blue-200 text-blue-600 py-3 rounded-xl font-bold hover:bg-blue-50 transition-colors shadow-sm"
                >
            ...
            {order.status === 'pending' && (
              <div className="flex gap-2">
                <button onClick={() => openRejectModal(order.ordersId || order.id)} className="flex-1 py-3 border border-red-200 text-red-500 bg-white rounded-xl text-[13px] font-black hover:bg-red-50 transition-colors">ปฏิเสธ</button>
                <button onClick={() => handleStatusChange(order.ordersId || order.id, 'cooking')} className="flex-[2] py-3 bg-[#ea580c] hover:bg-orange-600 text-white rounded-xl text-[14px] font-black shadow-sm transition-all active:scale-95">รับออเดอร์</button>
              </div>
            )}
            {order.status === 'cooking' && (
              <button onClick={() => handleStatusChange(order.ordersId || order.id, 'completed')} className="w-full py-3.5 bg-green-500 hover:bg-green-600 text-white rounded-xl text-[14px] font-black shadow-sm transition-all active:scale-95 flex justify-center items-center gap-2">
                <CheckCircle size={20}/> เสิร์ฟอาหารสำเร็จ
              </button>
            )}
        </div>
    </div>
