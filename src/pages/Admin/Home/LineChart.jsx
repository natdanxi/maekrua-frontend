        ...
        <div className="relative w-full h-[280px] mt-8 mb-6 font-sans">
            ...
            <div className="absolute inset-0 ml-[52px] z-10">
                <svg className="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 100">
                    ...
                    <polyline fill="none" stroke="#a855f7" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" points={getPoints('walkin')} />
                    <polyline fill="none" stroke="#3b82f6" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" points={getPoints('online')} />
                </svg>
                {data.map((d, i) => {
                    const x = (i / (data.length - 1)) * 100;
                    ...
                    return (
                        <React.Fragment key={i}>
                            <div className="absolute w-4 h-4 bg-white border-[3px] border-purple-500 rounded-full shadow-sm hover:scale-125 transition-all cursor-pointer group z-20" style={{ left: `calc(${x}% - 8px)`, top: `calc(${yWalkin}% - 8px)` }}>
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-gray-800 text-white text-[11px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-lg whitespace-nowrap z-50">฿{d.walkin.toLocaleString()}</div>
                            </div>
                            <div className="absolute w-4 h-4 bg-white border-[3px] border-blue-500 rounded-full shadow-sm hover:scale-125 transition-all cursor-pointer group z-20" style={{ left: `calc(${x}% - 8px)`, top: `calc(${yOnline}% - 8px)` }}>
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-gray-800 text-white text-[11px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-lg whitespace-nowrap z-50">฿{d.online.toLocaleString()}</div>
                            </div>
                        </React.Fragment>
                    );
                })}
