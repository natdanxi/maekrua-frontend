    ...
    <>
      ...
      <div className={`fixed top-0 left-0 w-[280px] h-full bg-white z-[110] shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {isAdmin ? (
            <>
              {/* 🟢 แก้ไขเส้นทางตรงนี้เป็น /admin/home เพื่อให้ตรงกับ Route จริง */}
              <Link to="/admin/home" onClick={() => setIsSidebarOpen(false)} className="flex items-center justify-between px-6 py-4 text-gray-700 hover:bg-gray-50 transition-colors border-b border-gray-50"><div className="flex items-center gap-4"><LayoutDashboard size={20} className="text-gray-500" /><span className="font-bold text-[15px]">แดชบอร์ด</span></div><ChevronRight size={16} className="text-gray-300" /></Link>
